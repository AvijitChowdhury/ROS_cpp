...
int main(int argc, char **argv) {
  ...
  // Rate is a class the is used to define frequency for a loop. Here we send a
  // message each two seconds.
  ...
  while (ros::ok()) // Keep spinning loop until user presses Ctrl+C
  {
    ...
    // Message definition in this link
    ...
    loop_rate
        .sleep(); // Sleep for the rest of the cycle, to enforce the loop rate
    count++;
  }
  return 0;
}
